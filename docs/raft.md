...
2. П отправляет `InstallSnapshotResponse` с успехом
    3. Если терм лидера меньше нашего, то ответить отрицательно
    4. Закончить обработку
3. Л отправляет следующий чанк снапшота
    - Если размер чанка - 0
        1. П отправляет успешный `InstallSnapshotResponse`
        2. Файл снапшота обновляется/создается новый (заменяется созданным)
        3. Файл лога очищается
        4. Работа заканчивается
...
4. П записывает чанк снапшота в файл
5. П отправляет успешный `InstallSnapshotResponse`
6. Повторяем с 3 шага
