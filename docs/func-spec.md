...
> От ответственности отказываюсь
...
Эта операция позволяет получить размер очереди. 
...
В ответе передается список всех очередей и их настройки. 
...
> Замечание: реализовано через uint, поэтому такие граничные значения.
Потом, может, изменю на `ushort` или типа того

## Приоритет

В качестве приоритета используются целые числа.
Диапазон от `-2147483648` до `2147483647` включительно.

> Подумать: изначально делал через `int`, но нужна ли поддержка различных типов ключей (например, строки или байты)?
