...
Используется, когда узел лидер хочет реплицировать состояние на узел последователь,
если в логе нет нужных записей.

Формат:

| Маркер    | Id узла | Последний индекс | Последний терм | Размер | Данные |
|-----------|---------|------------------|----------------|--------|--------|
| Byte('C') | Int32   | UInt32           | Term           | UInt32 | Byte[] |

Поля `Размер` и `Данные` составляют логическую единицу буфера.
Поле `Размер` указывает на количество байтов в поле `Данные`.
Если поле `Размер` равно 0, то это означает конец данных снапшота (и, соответственно, конец передачи)л

## InstallSnapshotResponse

Используется для ответа на InstallSnapshotRequest.
