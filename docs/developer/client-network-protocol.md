...
4. Клиент отправляет [Acknowledge](#acknowledge)
5. Сервер выполняет команду добавления записи
6. Сервер отправляет [Command Response](#command-response) пакет с результатом выполнения команды
...
sequenceDiagram
    ...
    Сервер->>Клиент: Ok
    Клиент->>Сервер: Acknowledge
    Сервер->>Клиент: CommandResponse
```

### Чтение

Это команда для получения записи из нужной очереди.
...
2. Сервер читает запись из очереди
3. Сервер отправляет [Command Response](#command-response) клиенту с прочитанной записью
4. Клиент отправляет [Acknowledge](#acknowledge) серверу
5. Сервер коммитит выполнение команды
...
> Пока коммит поддерживается только для Dequeue

## Ok

Пакет, сигнализирующий о принятом запросе.
В основном используется совместно с [Acknowledge](#acknowledge) для подтверждения выполнения команды.
...
Если необходимо, то дополнительное описание ошибки представлено в поле `Message`.

Формат:

| Marker    | Код ошибки | Дополнительная информация | 
|-----------|------------|---------------------------|
| Byte('e') | Int32      | String                    |
...
| Код ошибки | Описание                      |
|------------|-------------------------------|
| 1          | Неправильное название очереди |
