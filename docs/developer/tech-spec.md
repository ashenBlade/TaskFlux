...
- `Данные` - сериализованная данные команды для восстановления

Поля `Терм` и `Данные` образуют логическую единицу - команду (в соответствии с алгоритмом рафта).
Они повторяются до конца файла.
Никаких маркеров начала/конца команды нет - непримененные записи хранятся в памяти.

Поле `Данные` содержит сериализованное представление дельты изменения состояния приложения.
Это не сама команда, которая подается на вход приложению
...
В этом файле хранится слепок состояния приложения - множество всех очередей, которые существовали на момент его
создания.
