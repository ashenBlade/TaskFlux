...
public class SegmentedFileLogTests : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        {
            var fileLogCreation = () => SegmentedFileLog.Initialize(dataDir, Logger.None);
    ...
    // TODO: тесты когда часть записей нет в логе (compacted)
    ...
    public void InsertRangeOverwrite__КогдаПревышенМягкийПределИВсеЗаписиБылиЗакоммичены__ДолженНачатьНовыйСегмент()
    {
        ...
        var initialTailEntries = SegmentedFileLog.GenerateEntriesForSizeAtLeast(softLimit, 1);
