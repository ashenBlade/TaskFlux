...
namespace JobQueue.PriorityQueue.SkipList.Tests;

public class SkipListPriorityQueueTests
{
    private static SkipListPriorityQueue<int, int> CreateQueue() => new();
    ...
    [Fact]
    public void TryDequeue__КогдаВОчередьДобавляется2ЭлементаСОдинаковымиКлючами__ДолженВернутьПервоеДобавленноеЗначение()
    {
        var queue = CreateQueue();
        var first = ( Key: 1, Value: 2 );
        var second = ( Key: 1, Value: 3 );
