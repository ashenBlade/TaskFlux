...
[assembly: InternalsVisibleTo("Consensus.Peer.Tests")]
...
internal static class Serializers
{
    ...
    {
        public static byte[] Serialize(Consensus.Core.Commands.RequestVote.RequestVoteRequest request)
        {
        ...
        public static Consensus.Core.Commands.RequestVote.RequestVoteRequest Deserialize(byte[] buffer)
        {
            ...
            return new Consensus.Core.Commands.RequestVote.RequestVoteRequest(CandidateId: new(candidateId), CandidateTerm: new(candidateTerm),
    ...
    {
        public static byte[] Serialize(Consensus.Core.Commands.RequestVote.RequestVoteResponse response)
        {
        ...
        public static Consensus.Core.Commands.RequestVote.RequestVoteResponse Deserialize(byte[] buffer)
        {
            var stream = new MemoryStream(buffer);
            using var reader = new BinaryReader(stream);
            var marker = reader.ReadByte();
            ...
            return new Consensus.Core.Commands.RequestVote.RequestVoteResponse(CurrentTerm: new Term(currentTerm), VoteGranted: voteGranted);
        }
    }
    ...
    public static class AppendEntriesRequest
    {
        ...
        public static byte[] Serialize(Consensus.Core.Commands.AppendEntries.AppendEntriesRequest request)
        {
            // Маркерный байт + 4 значения по 4 байта
        ...
        public static Consensus.Core.Commands.AppendEntries.AppendEntriesRequest Deserialize(byte[] buffer)
        {
            ...
            if (entriesCount == 0)
            {
                return new Consensus.Core.Commands.AppendEntries.AppendEntriesRequest(LeaderId: new(leaderId), Term: new(term), LeaderCommit: commit,
    ...
    {
        public static byte[] Serialize(Consensus.Core.Commands.AppendEntries.AppendEntriesResponse response)
        {
            using var stream = new MemoryStream();
            using var writer = new BinaryWriter(stream);
        ...
        public static Consensus.Core.Commands.AppendEntries.AppendEntriesResponse Deserialize(byte[] buffer)
        {
            ...
            return new Consensus.Core.Commands.AppendEntries.AppendEntriesResponse(new Term(term), success);
        }
    }
